...
- **Scale-invariant depth error: ~2.0% relative RMS** vs PyTorch fp32 (full 24-block ViT-L + ConvStack decoder pipeline).
- Global metric scale differs by ~15% (uniform across the image). This is **not** weight-precision error: rerunning with fp32 weights reproduces the offset and the 2% figure essentially bit-for-bit, so the divergence is a deterministic difference between the WebGPU compute path and PyTorch. About half the offset (×1.077, measured by `tools/probe_scale_head.mjs`) enters through drift in the CLS token that feeds the metric-scale head; the remainder is under investigation, with the earliest measurable divergence already present at the patch-embed comparison stage.
- Per-layer tensor parity against PyTorch dumps (`tools/compare_backbone.mjs`): early transformer blocks match within rms 0.02–0.03; relative drift grows through depth to ~30% relative rms at block 23, concentrated in DINOv2's known high-norm outlier tokens — without materially moving the final depth structure (the 2% figure above).
